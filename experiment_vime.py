...
def run(game, path="result",  solvername = "vime_cfr", Type="default", resultpath="leduc_3_4_10"):
    
    def solve(gamesolver, reporttime=2, timelim = 3000, minimum=0, mrounds = 10000): # gamesolver是什么？？？怎么引过来的？
        cumutime = 0
        timestamp = time.time()
        result = [] # 结果
        expls = []  # 可利用度
        ...
        while rnds < mrounds: # 一共循环10000局
            ...
            if rnds % 50 == 0: # 每30轮就 计算并获取 一次可利用度
                ...
                np.savez( "/root/progs/submit/results/" + resultpath+"_"+solvername+"_"+Type + ".npz", expls = expls, rounds = rounds)
                print("results/" + resultpath+"_"+solvername+"_"+Type + ".npz" + " " + "保存成功")
                #  games/leduc_3_4_10
        return (expls,  rounds)   # solve函数返回的是 可利用度和轮次

    solver = None
    if solvername == "vime_cfr":
        solver = vime_cfr.VIMECFR(game, Type=Type)
