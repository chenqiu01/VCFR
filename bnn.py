...
PI_tensor = torch.Tensor([np.pi])#.to(device)
# ------------------------
...
class BNNLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.prior_sd = prior_sd#.to(device)

        prior_rho = self.std_to_log(prior_sd)
        ...
        self.b = torch.Tensor(np.zeros(
            (self.num_units,),
            dtype=np.float)) # @UndefinedVariable
        ...
        self.b_mu_old = torch.Tensor(np.zeros((self.num_inputs)))
    ...
    def get_output_for_reparametrization(self, input, **kwargs):
        ...
        epsilon = torch.Tensor(self.num_units, )#.to(device)
        torch.nn.init.normal_(epsilon, mean=0., std=1.)
    ...
    def get_output_for_default(self, input, **kwargs):
        ...
        activation = torch.mm(input, self.get_W()) + self.get_b().expand(input.size()[0], self.num_units)
...
class BNN(nn.Module):
    ...
    def __init__(self, n_in,
        ...
        self.reverse_kl_reg_factor = reverse_ki_reg_factor
        self.likelihood_sd = torch.Tensor([likelihood_sd])#.to(device)
    ...
    def _log_prob_normal(self, input, mu=0.,
        ...
        log_normal = - torch.log(sigma) - torch.log(torch.sqrt(2 * PI_tensor)) - square(input - mu)/(2 * square(sigma))
    ...
    def loss(self, input, target):
        ...
        if self.use_reverse_kl_reg:
            kl += self.reverse_kl_reg_factor * self.reverse_log_p_w_q_w_kl()
