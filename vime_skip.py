...
class VIME():
    def __init__(self,
        ...
        self.old_kl = torch.zeros(input_num,1)
        self._inputs_pos = range(input_num)
    ...
    def update_reward(self, rewards):
        if self.iter > 0:
            if self.iter % 10 == 0:
                self._inputs_pos = range(self.input_num)
            ...
            _rews = []
            _labels = []
            for i in self._inputs_pos:
                _rews.append(rewards[self.postions[i]][1])
                _labels.append(self._inputs[i])

            
            _inputs = torch.stack(_labels).float()
            ...
            if num_processes == 1:
                ...
                compute_intrinsic_reward(dynamics, 0, _inputs, _targets, kl, step, kl_batch_size,
            ...
            poss=[]
            for i, pos in enumerate(self._inputs_pos):
                self.old_kl[pos] = kl[i]
                if kl[i] > 1:
                    poss.append(pos)
            ...
            for i, pos in enumerate(self.postions):
                r = rewards[pos][1] +self.eta * kl[i]
                rewards[pos][0] = -r
                rewards[pos][1] = r
...
def resample():
    num =400
    rewObs = list(map(lambda _u: np.ones(2), range(num)))
    rews = []
    for i in range(num):
        if i%2:
