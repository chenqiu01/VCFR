...
def run(game, path="result",  solvername = "vime_discount", Type="br_dirc", resultpath="leduc_3_5_10"):
	...
	def solve(gamesolver, reporttime=2, timelim = 3000, minimum=0, mrounds = 20000): # gamesolver是什么？？？怎么引过来的？
		...
		while rnds < mrounds: # 一共循环10000局
			...
			if rnds % 50 == 0: # 每30轮就 计算并获取 一次可利用度
				...
				np.savez( "./submit/results/" + resultpath+"_"+solvername+"_"+Type + ".npz", expls = expls, rounds = rounds)
	...
	if solvername == "vime_discount":
		solver = skip_dcfr_avg.VIME_DIS(game,  3/2 , 0 ,2 ,Type=Type)
